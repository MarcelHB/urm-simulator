...
//-----------------------------------------------------------------------------
void accept(URMProgram *program, FILE *stream) {
...
int parse_positive_int(URMProgram *program, FILE *stream) {
    ...
    while(i < 10 && program->current_char >= 48 &&
...
unsigned int parse_operation(URMProgram *program, FILE *stream) {
    ...
    if(number >= 0) {
        ...
        *(urmi->arg_list) = number;
        ...
        accept(program, stream);

        return 1;
    } else {
        add_error(program, "[parsing] NaN for operation register");
        return 0;
    }
}
...
unsigned int parse_iteration_end(URMProgram *program, FILE *stream) {
    ...
    if(program->current_char == URM_SYMBOL_IT_END && marker->token == ITERATION) {
        ...
        *(urmi->arg_list) = ((int)distance) * -1;
        ...
        if(count_register == -1) {
            add_error(program, "[parsing] no operation register");
            ...
            while(i != program->instructions && loop_style) {
                switch(program->instruction_list[i]->instruction) {
                    case OP_INC:
                        if(program->instruction_list[i]->arg_list[0] == count_register) {
                            add_error(program, "[compiler] attempting to increase the count register");
                            loop_style = 0;
                        }
                        break;
                    case OP_DEC:
                        if(program->instruction_list[i]->arg_list[0] == count_register) {
                            if(i != program->instructions - 2) {
                                add_error(program, "[compiler] decreasing of counter at wrong location");
                                loop_style = 0;
                            }
                        }
                        break;
                ...
                if(i == program->instructions - 2) {
                    if(program->instruction_list[i]->instruction != OP_DEC || 
                        ...
                        add_error(program, "[compiler] not decreasing the counter");
                        loop_style = 0;
                    }
                }
                ++i;
    ...
    while(parsing) {
        ...
        if(number != -1) {
            ...
            *destinations = (unsigned int*)realloc(*destinations, n * sizeof(unsigned int));
            ...
            add_error(program, "[parser] expected destination");
            break;
        }
        ...
        switch(program->current_char) {
            ...
            case URM_SYMBOL_COMMA:
                ...
                break;
           default:
                add_error(program, "[parser] expected ',' or ')'");
                parsing = 0;
                break;
        }
    }
...
unsigned int parse_operation_with_args(URMProgram *program, FILE *stream) {
    ...
    if(program->current_char == URM_SYMBOL_IT_BEGIN) {
        ...
        if(source_register == -1) {
            add_error(program, "[parser] no source register");
            return 0;
        }
        ...
        switch(program->current_char) {
            ...
            case URM_SYMBOL_SEPERATOR:
                ...
                if(program->current_char == URM_SYMBOL_IT_END) {
                    accept(program, stream);
                } else {
                    free(destination_list);
                    add_error(program, "[parser] unclosed iteration");
                    return 0;
                }
                break;
            default:
        ...
        add_error(program, "[parser] missing operation arguments");
        return 0;
    }
}
...
unsigned int next_token(URMProgram *program, FILE *stream) {
    ...
    if(program->stack_size == 0) {
        success = parse_operations(program, stream);
    } else {
        URMStackMarker *marker = top_parsing_stack(program);
        if(marker->token == ITERATION) {
            success = parse_operations(program, stream);
            if(!success) {
                success = parse_iteration_end(program, stream);
            }
        }
    }
...
unsigned int parse(URMProgram *program, FILE *stream) {
    ...
    if(ftell(stream) > program->consumed && program->errors == 0) {
      add_error(program, "[parser] garbage detected");
      return 0;
    }
    ...
    *urmi = (URMInstruction)  { 
    ...
    if(program->stack_size > 0 && program->errors == 0) {
        add_error(program, "[parser] unclosed iteration");
        return 0;
    }
